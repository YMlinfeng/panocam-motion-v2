...
1. 90 个由 2～6 种旋转/变焦基础动作组成的连续混合时序：45 个动作配方各生成 A/B 两个独立变体，每条都有覆盖 15 秒的持续 backbone，并叠加长短不一的错峰动作。
...
速度、幅度、动作起止、持续时间和曲线均随机化，并人工加入“快速为主、范围较大”的偏置；高难度覆盖甩镜、大范围甩镜和 360°滚转环绕。360+x 官方人脸模糊素材只分配到 REF，Wikimedia Commons / NASA 素材分配到 TARGET。
...
两者是彼此独立的 Git 仓库。新版不会读取旧版代码或旧 demo；本机只读引用旧目录中的 360+x 5K 数据，新增 Commons/NASA 原始素材统一放在当前项目的 `local/sources/commons`。公开仓库配置提供了改成本地其他路径的方法。
